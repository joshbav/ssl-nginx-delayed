...
shutdown() {
  ...
  echo
  echo "Shutting down gracefully in $SIGKILL_DELAY_SECONDS seconds."
  echo "Removing /healthy.html endpoint /usr/share/nginx/html/healthy.html"
  rm -f /usr/share/nginx/html/healthy.html
  echo
  echo "Sleeping for $SIGKILL_DELAY_SECONDS seconds so health check can fail and the load balancer can"
  echo "withdrawl the app."
  sleep SIGKILL_DELAY_SECONDS
  echo
  echo Exiting
  echo
  exit 0
...
then
   echo No STARTUP_DELAY_SECONDS varible set, defaulting to 30.
...
echo
if [ -z "$SIGKILL_DELAY_SECONDS" ]
then
   echo No SIGKILL_DELAY_SECONDS varible set, defaulting to 45.
   SIGKILL_DELAY_SECONDS=45
fi

echo SIGKILL_DELAY_SECONDS is $SIGKILL_DELAY_SECONDS seconds.
...
echo
echo Done sleeping, now starting NGINX. Ignore the SSL warning. 
echo
...
echo
echo RUN_TIME_SECONDS has elapsed, now calling SIGTERM handler function.
shutdown
